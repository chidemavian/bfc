...
def endtermstats(session,term,klass,arm):
    ...
    for j in allstu:
        ...
        if session =='2023/2024':
            if term == 'First' :
                if arm[0] == 'J':
                    mysub = SubjectScore.objects.filter(academic_rec = acaderec,term = term).exclude(subject='MUSIC')
                else:
                ...
                if arm[0] == 'J':
                    ...
                    mysub = SubjectScore.objects.filter(academic_rec = acaderec,term = term).exclude(subject='GEOGRAPHY')
...
def classaveragemid_old(klass,session,term,arm):
    if term == 'Third':
        academicins = StudentAcademicRecord.objects.get(term = term,session = session,klass = klass,arm = arm)
