            ...
            if form.is_valid():
                ...
                if term == 'First':
                    ...
                    try:
                        student = Student.objects.get(admitted_session =session,
                            first_term = 1,
                            # admissionno='BFC/21/0274/7952',
                            fullname=varuser,
                            gone = False)

                    except:
                        msg = 'You do not have first term records1'
                ...
                if term == 'Second':
                    ...
                    try:
                        ...
                        msg = 'You do not have first term records2'
                        return render_to_response('assessment/selectloan_student.html',{'msg':msg,'user':varuser})
                ...
                if term == 'Third':
                    ...
                    try:
                        ...
                        msg = 'You do not have first term records3'
                        return render_to_response('assessment/selectloan_student.html',{'msg':msg,'user':varuser})
                ...
                if report== 'End term':
                    ...
                    klass = str(student.admitted_class)
                    arm=str(student.admitted_arm)
...
def editcas2bothtypes(request,vid):
    ...
    #          aklm = SubjectScore.objects.filter(session = session1,term = term1,klass = klass1,arm = arm1,subject = subject1)


    #          #************TOTAL STUDENT IN CLASS offering subject************
    #          totstudent = aklm.count()

    #          #********************endterm score total****************
    #          totsubject = aklm.aggregate(Sum('end_term_score'))
    #          varrid = totsubject['end_term_score__sum']

    #          subavg = varrid/totstudent

    #          aklm.update(subject_teacher = uenter.upper(),subject_avg = subavg)


    #          annavg = 0
    #          if term1 == 'Third':
    #              an = annualaverage(str(admno1),str(session1),str(arm1),str(klass1),str(subject1))



    #          #***************************getting subject position**************************#
    #          sp = subjectposition(str(session1),str(subject1),str(term1),str(klass1),str(arm1))
    ...
    #          #******************getting mid term classroom average*****************
    #          ca=classaverageEnd(klass1,session1,term1,arm1)

    #          sa=studentaverageEnd(admno1,term1,session1,klass1,arm1)


    #          #***********************getting the classroom position*******
    #          cp = classposition(str(session1),str(term1),str(klass1),str(arm1))


    #          #************getting class position****************
    #          cp1 = classposition1(str(session1),str(term1),str(klass1))
             c = klass1[0]
...
#                 return HttpResponseRedirect('/reportsheet/addsubject/')
#             else:
...
def reportsheet(request):
 ...
 ##OBTAINING SECOND TERM RECORDS *******************
                        ...
                        'subject':secsublist,
                         'st_grade':stat,
                         'scor':locale.format("%.2f",stave,grouping=True),
                        'totalmark':rtotals,
                    ...
                    if klass[0] == 'S':
                        ...
                        return render_to_response('assessment/reportthirdsss.html',
                            {'varerr':varerr,'replist':replist,'school':school,'date':date,'term':term,'next':ntb})
                    elif klass[0] == 'N' or klass[0] == 'C' or klass[0] == 'L':
                        # if form.cleaned_data['pdffile']:
                        #     template ='assessment/reportnviewthird.html'
                        #     context = {'form':form,'varerr':varerr,'replist':replist,'school':school,'term':term,'stuno':stuno,'classavg':locale.format("%.2f",clavg,grouping=True)}
                        #     return render_to_pdf(template, context)
                        # else:
                        return render_to_response('assessment/reportnthird.html',{'form':form,'varerr':varerr,'replist':replist,'date':date,'school':school,'term':term,'stuno':stuno,'classavg':locale.format("%.2f",clavg,grouping=True)})
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if klass.startswith('S'):
                   ...
                   wb.save(response)
                return response
                # return HttpResponseRedirect('/reportsheet/broadsheet/','response':response)
