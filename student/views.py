...
from bfc.student.forms import *


#from bfc.student.utils import render_to_pdf, render_to_xls
from bfc.sysadmin.admin import *
...
def welwb(request): #add further maths to all students in select class
    klass=['SS 1', 'SS 2', 'SS 3']
    armz= ['ART','COMMERCIAL']
    ...
    for k in klass:
        ...
        for arm in armz:
            ...
            num = Subject.objects.filter(klass=g,category=category)
            num_count=num.count()


            if num.filter(subject=subject,category=category,klass=g).count() ==0:
            ...
            all_students= Student.objects.filter(admitted_session='2023/2024',
                admitted_class = k,
                admitted_arm=arm,
                gone=0,
                second_term=1 )
            ...
            for s in all_students:
                for tt in term:
                    f= StudentAcademicRecord.objects.get(student=s,
                    klass=k,arm=arm, term=tt,session=sessssion)

                    sub = SubjectScore.objects.filter(academic_rec=f,
                        term=tt,klass=k,arm=arm,session=sessssion)

                    if sub.filter(subject=subject).count() == 0:
                        SubjectScore(
                        academic_rec = f,
                        subject = subject,
                        subject_group = 'ALL',
...
def wel1(request): #add further maths to all students in select class
  code = 'BFC/21/0328/4526'
  session='2023/2024'
  term =['Second', 'Third']
  stu = Student.objects.get(admitted_session=session,admissionno=code)

  stu.admitted_arm='ART'
  stu.subclass='Art'
  stu.save()
  ...
  for t in term:
    rec= StudentAcademicRecord.objects.get(student=stu,term=t)
    rec.arm='ART'
    rec.save()
  # return render_to_response('student/wel.html',{'varuser':rec.arm})
    sub = SubjectScore.objects.filter(academic_rec=rec,term=t)
    sub.update(arm='ART')
    ...
    for s in sub:
      if s.subject=='T.D':
        ...
        s.num=14
        s.save()
      elif s.subject=='CHEMISTRY':
        s.subject='YORUBA'
        s.num=12
        s.save()
      elif s.subject=='PHYSICS':
        s.subject='CRS'
        s.num=10
        s.save()

      elif s.subject=='GEOGRAPHY':
        s.subject='LITERATURE'
        s.num=11
        s.save()
...
def welt(request):#fillup
    sessionnew = '2023/2024'
    terrm = ['First', 'Second', 'Third']
    mm = Student.objects.filter(admitted_session=sessionnew,gone=0,admitted_class='SS 1')

    for newstu in mm :

      for term in terrm:
          StudentAcademicRecord.objects.filter(student=newstu,term=term,session=sessionnew).update(klass=newstu.admitted_class,arm=newstu.admitted_arm)
...
def weltr5(request): #corect class arm

    k = Student.objects.get(admissionno='BFC/u21/0217/8537')
    di = StudentAcademicRecord.objects.get(student=k,term='Third')
    sdf = SubjectScore.objects.get(academic_rec=di,subject='CRS')
    sdf.arm='A'
    sdf.save()
